...
연세대학교 상경대학 서버는 여러 대의 서버를 하나의 클러스터처럼 묶어서 사용하는 HPC(High Performance Computing) 환경입니다.  
사용자는 각 서버를 개별적으로 관리할 필요 없이, 하나의 시스템 안에서 Python, R, TensorFlow 등의 작업을 수행할 수 있습니다.

상경대학 서버는 상경대학 구성원을 위한 공용 계산 자원입니다.
다만 서버 자원은 구축 재원 및 운영 목적에 따라 이용 대상이 다를 수 있으며, 사용 가능한 서버와 자원 범위는 계정 발급 시 안내됩니다.

현재 상경대학 서버는 다음과 같이 구성되어 있습니다.
...
상경대학 서버는 개인용 컴퓨터와 달리 여러 사용자가 함께 사용하는 공용 계산 환경입니다.  
...
이러한 문제를 줄이기 위해 상경대학 서버는 **Slurm job scheduler**를 사용합니다.
...
따라서 상경대학 서버에서는 Python, R, TensorFlow 등 계산 작업을 **원칙적으로 Slurm을 통해 실행**합니다.
...
상경대학 서버에서는 패키지 충돌을 방지하고 환경을 안정적으로 관리하기 위해, 사용자별 독립 실행 환경(가상환경)을 사용합니다.  
